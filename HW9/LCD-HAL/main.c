...
void show_freq(void)
{
	...
	snprintf(str, sizeof(str), "%d", BaseFreq);
...
void ADC_CONTROLLER(void)
{
	...
	else if ((ADC_Result > 2900 && ADC_Result < 3100) && (select_flag))
	{
		// decrease dc (left)
		Duty_Cycle -= step;
		if (Duty_Cycle <= 0)
	...
	else if ((ADC_Result > 800 && ADC_Result < 900) && (!select_flag))
	{
		// increase period (up)
		Update_Freq(true);
		show_freq();
	...
	else if ((ADC_Result > 1900 && ADC_Result < 2100) && (!select_flag))
	{
...
void PWM_CONTROLLER()
{
	// for (unsigned int d = 0; d < 200000; d++)
	// 	; // delay
